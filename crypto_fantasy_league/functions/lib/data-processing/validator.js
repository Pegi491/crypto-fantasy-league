...
class DataValidator {
    ...
    static validateWalletBalance(data) {
        const errors = [];
        const warnings = [];
        const sanitizedData = Object.assign({}, data);
        // Validate address
    ...
    static validateSocialMetrics(data) {
        const errors = [];
        const warnings = [];
        const sanitizedData = Object.assign({}, data);
        // Validate symbol
...
DataValidator.MIN_PRICE = 0.000001; // Minimum valid price (1 millionth)
DataValidator.MAX_PRICE = 10000000; // Maximum valid price (10M)
DataValidator.MIN_VOLUME = 0;
DataValidator.MAX_VOLUME = 1000000000000; // 1T
DataValidator.MIN_MARKET_CAP = 0;
DataValidator.MAX_MARKET_CAP = 10000000000000; // 10T
DataValidator.MAX_CHANGE_PERCENT = 10000; // 10000% (for extreme meme coins)
DataValidator.MAX_AGE_MINUTES = 30; // Data must be less than 30 minutes old
async function validateAssetData(data) {
    firebase_functions_1.logger.info("Validating asset data", {
        dataType: typeof data,
        hasSymbol: !!(data === null || data === void 0 ? void 0 : data.symbol),
    });
    try {
        let result;
        ...
        firebase_functions_1.logger.info("Data validation completed", {
        ...
        firebase_functions_1.logger.error("Data validation failed", error);
        return {
            isValid: false,
