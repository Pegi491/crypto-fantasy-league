...
export class AssetMetadataUpdater {
  private static readonly POPULAR_THRESHOLD_VOLUME = 1000000; // $1M daily volume
  private static readonly POPULAR_THRESHOLD_MARKET_CAP = 10000000; // $10M market cap
  private static readonly STALE_DATA_HOURS = 24; // Data older than 24 hours is stale

  static async updateAssetPopularity(asset: any): Promise<boolean> {
    try {
      ...
      const isPopular = 
        volume24h >= this.POPULAR_THRESHOLD_VOLUME ||
      ...
      if (asset.isPopular !== isPopular) {
        ...
        logger.info("Updated asset popularity", {
          assetId: asset.id,
          symbol: asset.symbol,
          isPopular,
          volume24h,
      ...
      logger.error("Failed to update asset popularity", {
        assetId: asset.id,
  ...
  static async updateAssetRanking(asset: any): Promise<boolean> {
    try {
      const metadata = asset.metadata || {};
      ...
      if (asset.type === "token" && marketCap > 0) {
        ...
        if (newRank !== currentRank) {
          ...
          logger.debug("Updated asset ranking", {
            assetId: asset.id,
            symbol: asset.symbol,
            oldRank: currentRank,
            newRank,
      ...
      logger.error("Failed to update asset ranking", {
        assetId: asset.id,
        error: error instanceof Error ? error.message : "Unknown error",
      });
      throw error;
  ...
  static async updateDataFreshness(asset: any): Promise<boolean> {
    try {
      const now = admin.firestore.Timestamp.now();
      const staleThreshold = admin.firestore.Timestamp.fromMillis(
        now.toMillis() - this.STALE_DATA_HOURS * 60 * 60 * 1000
      );
      ...
      if (metadata.price_usd) completenessScore += 25;
      if (metadata.volume_24h) completenessScore += 25;
      if (metadata.market_cap) completenessScore += 25;
      if (metadata.social_score) completenessScore += 25;
      ...
      if (hasUpdates) {
        ...
        logger.debug("Updated data freshness", {
          assetId: asset.id,
          symbol: asset.symbol,
          completenessScore,
      ...
      logger.error("Failed to update data freshness", {
        assetId: asset.id,
        error: error instanceof Error ? error.message : "Unknown error",
      });
      throw error;
  ...
  static async enrichAssetMetadata(asset: any): Promise<boolean> {
    try {
      const metadata = asset.metadata || {};
      const updates: any = {};
      let hasUpdates = false;
      ...
      if (hasUpdates) {
        ...
        logger.debug("Enriched asset metadata", {
          assetId: asset.id,
          symbol: asset.symbol,
      ...
      logger.error("Failed to enrich asset metadata", {
        assetId: asset.id,
        error: error instanceof Error ? error.message : "Unknown error",
      });
      throw error;
  ...
  private static calculateRiskScore(metadata: any): number {
    let riskScore = 50; // Start with neutral risk

    // Market cap factor (lower market cap = higher risk)
    if (metadata.market_cap) {
      if (metadata.market_cap < 100000000) riskScore += 20; // <$100M
      else if (metadata.market_cap < 1000000000) riskScore += 10; // <$1B
      else if (metadata.market_cap > 10000000000) riskScore -= 10; // >$10B
    }

    // Volatility factor
    if (metadata.volatility7d) {
      if (metadata.volatility7d > 50) riskScore += 15; // High volatility
      else if (metadata.volatility7d < 10) riskScore -= 10; // Low volatility
    }
    ...
    if (metadata.volume_24h && metadata.market_cap) {
      ...
      if (volumeRatio < 0.01) riskScore += 15; // Very low liquidity
      else if (volumeRatio > 0.5) riskScore += 10; // Suspiciously high volume
    }

    // Social sentiment factor
    if (metadata.social_score) {
      if (metadata.social_score < 30) riskScore += 10; // Poor sentiment
      else if (metadata.social_score > 80) riskScore -= 5; // Good sentiment
    }
...
export async function updateAssetMetadata(): Promise<UpdaterResult> {
  ...
  try {
    ...
    logger.info(`Processing ${assetsSnapshot.size} assets for metadata update`);
    ...
    logger.info("Asset metadata update completed successfully", {
      assetsUpdated,
